...
[АЛГОРИТМ ДЕЙСТВИЙ]
1.  **АНАЛИЗ ДОКУМЕНТА:** Ты симулируешь анализ документа, определяя тип контрагента (физическое лицо, ИП, юридическое лицо) и запрашивая подтверждение.
2.  **АДАПТИВНЫЙ СБОР ДАННЫХ:** Твой диалог адаптируется под тип контрагента.
3.  **ТРАНСФОРМАЦИЯ ЗАПРОСА:** Это твоя ключевая компетенция. Когда пользователь пишет запрос, ты должен:
    *   **Распознавать и расшифровывать сокращения:** "Упр.дир." должно превращаться в "Управляющий директор".
    *   **Распознавать и полностью расписывать денежные суммы:** "50к" или "200тыс" должно превращаться в "50 000 (пятьдесят тысяч) тенге".
    *   **Формализовать суть:** Весь запрос облекается в юридически корректную форму.
    *   **Пример трансформации:** Запрос "изменить премию на 50к" ты преобразуешь в ответ: "Ваш запрос принят. Он будет изложен в следующей редакции: 'Стороны договорились увеличить страховую премию на 50 000 (пятьдесят тысяч) тенге'".
4.  **ФИНАЛИЗАЦИЯ:** После сбора всех данных, ты формируешь итоговый документ.`;
    ...
    function simulateCorrection(text) {
        ...
        // 1. Expand Job Titles and other abbreviations
        const abbreviations = {
            "упр дир": "Управляющий директор", "упр.дир.": "Управляющий директор",
            "ген дир": "Генеральный директор", "ген.дир.": "Генеральный директор",
            "фин дир": "Финансовый директор", "фин.дир.": "Финансовый директор",
            "дир": "Директор",
            "ТОО": "Товарищество с ограниченной ответственностью",
            "АО": "Акционерное общество",
        };
        ...
        // 2. Expand monetary values
        const moneyRegex = /(\d+)\s*(к|тыс|тысяч)\b/gi;
        correctedText = correctedText.replace(moneyRegex, (match, number, suffix) => {
            const num = parseInt(number, 10) * 1000;
            return `${num.toLocaleString('ru-RU')} (${numberToWordsRu(num)}) тенге`;
        });

        // 3. Wrap in a formal sentence structure
        return `Стороны договорились изложить пункт Договора, касающийся изменений, в следующей редакции: "${correctedText}"`;
    }
